...
class ParallelEnv:
    def __init__(self, env_id, global_idx,
        ...
        # global_actor_critic.load_state_dict(torch.load('checkpoints/saved_model.pth'))
        global_optim = SharedAdam(global_actor_critic.parameters(), lr=1e-3)

        if icm:
            global_icm = ICM(input_shape, n_actions)
            global_icm.share_memory()
            # global_icm.load_state_dict(torch.load('checkpoints/saved_icm_model.pth'))
            global_icm_optim = SharedAdam(global_icm.parameters(), lr=1e-3)
        else:
            global_icm = None
